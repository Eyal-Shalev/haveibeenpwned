...
class Client {
  ...
  public function getAccountBreaches($account_name, $domain = NULL, $truncate = FALSE) {
    ...
    return $this->decodeResponse($this->getResponse($target_items, $options));
  }
  ...
  protected function decodeResponse(ResponseInterface $response) {
    switch ($response->getStatusCode()) {
      ...
      case 400:
        throw new \HttpResponseException('Bad request — the account does not comply with an acceptable format.');
      case 403:
        throw new \HttpResponseException('Forbidden — no user agent has been specified in the request.');
      case 404:
        throw new \HttpResponseException('Not found — the account could not be found and has therefore not been pwned.');
      default:
        return [];
    }
  }
  ...
  protected function getResponse(array $target_items, array $options = []) {
    if (empty($target_items)) {
      throw new \BadMethodCallException('The targets_items array cannot be empty.');
    }
  ...
  public function getAllBreaches($domain = NULL) {
    ...
    return $this->decodeResponse($this->getResponse(['breaches'], $options));
  }
  ...
  public function getBreach($site_name) {
    return $this->decodeResponse($this->getResponse(['breach', $site_name]));
  }
  ...
  public function getDataClasses() {
    if (empty(static::$dataClasses)) {
      static::$dataClasses = $this->decodeResponse($this->getResponse(['dataclasses']));
    }
